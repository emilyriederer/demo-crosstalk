...
```{r mult-choice-1, echo = FALSE}
question("Which of the following is the correct way to create the SharedData object?",
  answer("trips_ct <- SharedData$new(trips, group = ~station_id)", message = "~station_id should be passed as the argument to key"),
  answer("trips_ct <- SharedData$new(trips)", message = "key must be specified"),
  answer("trips_ct <- SharedData$new(trips, key = ~station_id)", correct = TRUE),
  answer("trips_ct <- SharedData$new(trips, key = 'station_id')", message = "station_id must be part of a formula"),
  allow_retry = TRUE
)
```
...
```{r mult-choice-2, echo = FALSE}
question("Which datasets will share information between them?",
  answer("data1_ct, data2_ct", correct = TRUE, message = "Yes! These have the same group name"),
  answer("data1_ct, data3_ct", message = "Wrong - these have the same key but what we need is the same group name"),
  answer("data2_ct, data3_ct", message = "Wrong - these use the same dataset but what we need is the same group name"),
  allow_retry = TRUE
)
```
